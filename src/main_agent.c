...
Agent create_agent(float x, float y)
{
 return (Agent) {
  ...
  .color = (SDL_Color) { AGENT_COLOR_RED, AGENT_COLOR_GREEN, AGENT_COLOR_BLUE, AGENT_COLOR_ALPHA },
...
{
 if (agent->rect.y + agent->rect.h > platform->rect.y // Bottom of agent lower than top of platform
        && agent->rect.y < platform->rect.y + platform->rect.h // Top of agent higher than bottom of platform
        && agent->rect.x + agent->rect.w > platform->rect.x // Right side fo agent further than left side of platform
        && agent->rect.x < platform->rect.x + platform->rect.w) // Left side of agent closer than right side of platform
 {
  if (agent->prev_pos.x + agent->rect.w <= platform->rect.x) // Hit platform from left side
  {
   agent->rect.x = platform->rect.x - agent->rect.w;
   agent->velocity.x = 0;
  }

  else if (agent->prev_pos.x >= platform->rect.x + platform->rect.w) // Hit platform from right side
  {
   agent->rect.x = platform->rect.x + platform->rect.w;
   agent->velocity.x = 0;
  }

  else if (agent->prev_pos.y >= platform->rect.y + platform->rect.h) // Hit platform from below
  {
   agent->rect.y = platform->rect.y + platform->rect.h;
   agent->velocity.y = 0;
  }
  else if (agent->prev_pos.y + agent->rect.h <= platform->rect.y) // Hit platform from above
  {
